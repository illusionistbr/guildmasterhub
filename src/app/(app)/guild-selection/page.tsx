...
import { ShieldPlus, Users, Home, Eye } from 'lucide-react';
...
export default function GuildSelectionPage() {
  ...
  if (isLoading) {
    return (
      <div className="space-y-8">
        ...
        <div>
          <h2 className="text-2xl font-headline text-primary mb-4">Suas Guildas</h2>
  ...
  const allUserGuilds = [...ownedGuilds, ...memberGuilds];
                  ...
                  <CardContent className="p-4 flex items-center justify-between gap-4">
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        ...
                        <p className="text-sm text-muted-foreground">{guild.ownerId === user.uid ? "Você é o Dono(a)" : "Você é Membro"}</p>
                      </div>
                    </div>
                    <Button variant="ghost" size="sm" className="text-primary hover:text-accent">
                      Acessar Painel <Eye className="ml-2 h-4 w-4" />
                    </Button>
                  </CardContent>
                </Link>
              </Card>
